...
In stage two, multiple images from different time stamps are used and processed by a **ResNet-50** backbone to obtain the feature map $$\mathbf{\mathit{F^{2D}_t}}$$, which serves as keys and values for the attention module. Only occupied voxels (blue) from the predicted occupancy grid are used as query input, demonstrating the paper's second key contribution of "**sparsity-in-3D-space**". Now, let's address the topic of Deformable Attention.
...
In the above expression, each query $$\mathbf{\mathit{{q}}}$$ is updated with the feature map $$\mathbf{\mathit{{F}}}$$ at the location of the corresponding point $$\mathbf{\mathit{{p}}}$$. Deformable attention extends beyond the corresponding point, also considering the surrounding area (8 surrounding points) using learnable offsets $$\mathbf{\mathit{{\delta p_s}}}$$. Interpolation is used to calculate values at these locations. The attention weight $$\mathbf{\mathit{{A_s}}}$$ is normalized between [0,1], while $$\mathbf{\mathit{{W_s}}}$$ represents the $$\mathbf{\mathit{{V}}}$$ matrix in the standard attention mechanism. Let's explore how this is applied in the paper.
<div align="center">
...
As mentioned before, the extracted image features are used as $$\mathbf{\mathit{{K}}}$$, $$\mathbf{\mathit{{V}}}$$ while the occupied voxels are used as $$\mathbf{\mathit{{Q}}}$$. Again, each query $$\mathbf{\mathit{{q_p}}}$$ from the voxel grid will be updated with the feature map $$\mathbf{\mathit{{F^{2D}_t}}}$$ at the location $$\mathcal{P}(\mathbf{\mathit{{p},t}})$$. The presence of the time stamp $t$ in this equation is crucial because each point in space is visible in specific images only. For example, in the image above, the black car within the orange box is visible solely in frames $t$ and $t+1$, so it makes sense that those queries from that car should only attend to the images where they appear on. Those images are indexed with $t$ and grouped as $$\mathbf{\mathit{{\mathcal{V}_t}}}$$. 

### Deformable Self-Attention
Next, the updated queries are combined with mask tokens $$\mathbf{\mathit{{m}}}$$, which account for empty or occluded voxels. Each query $$\mathbf{\mathit{{f}}}$$, representing either a mask token or an updated query proposal, is then updated with the feature map $$\mathbf{\mathit{{F^{3D}}}}$$ at its location and neighboring points. This aggregates image features from the updated queries to the remaining voxel regions. The resulting 3D feature map is upscaled and passed through a few fully connected layers to generate the final semantic output. The loss function employed is the weighted cross-entropy loss.
